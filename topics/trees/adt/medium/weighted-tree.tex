...
def total_weight(t):
  '''
  Return the total weight of a tree (sum of all labels).
  >>> total_weight(tree(1, [tree(2), tree(3, [tree(4)])]))
  10
  '''
  weight = label(t) + sum(total_weight(b) for b in branches(t))
  return weight
...
\begin{lstlisting}
def equally_weighted(t):
  all_weights = [total_weight(b) for b in branches(t)]
...
\begin{lstlisting}
def num_eq_weight(t):
  val = sum(num_eq_weight(b) for b in branches(t))

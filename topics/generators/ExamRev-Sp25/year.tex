\begin{blocksection}
    ...
    \begin{blocksection}
        ...
        \item We want the function to print 2, 0, 2, and 6 in that order. We know that the x iterator is pointing to the element at index 1 of the list, so the element at index 1 must be a 2. Calling print(next(x)) should print 2 and move the x iterator to point to the element at index 2.
        ...
        \item Lastly, we look at the z iterator (the same as the x iterator) and see its position is under the item at index 3, so we know that the item at index 3 should be 6. 
        \end{itemize}
    \end{blocksection}
    \end{guide}
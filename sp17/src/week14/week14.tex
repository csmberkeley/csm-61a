...
\begin{parts}
\part You and your friends are preparing for the 61A final by streaming lectures. You get tired and want to take a rest from studying but first you realize you are hungry so you check the refrigerator. You notice that \textbf{every other food} (starting with the first food in the list) in there is stale! Write a function that takes in a \texttt{Linked List} of foods,
called \texttt{foods} and outputs a stream that contains all your stale food.
...
\begin{solution}[20mm]
    \begin{lstlisting}[language=Python]
def stale_foods(foods):
  if foods is Link.empty:
    return Stream.empty
  if foods.rest is Link.empty
    return Stream(foods.first)
